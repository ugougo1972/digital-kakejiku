...
本プロジェクトは現行Phase体系を採用する。詳細な旧Phase体系との対応、現在位置、タスク優先度は `CURRENT_STATUS.md` を正とする。

| Phase | 内容 | 進捗 |
|---:|---|---|
| Phase 1 | GAS基盤本実装 | 次作業 |
| Phase 2 | Calendar Subsystem | 後続 |
| Phase 3 | ESP32統合 | 後続 |
| Phase 4 | Poem Subsystem | 後続 |
| Phase 5 | 長期運用評価 | 後続 |
| Phase 6 | 筐体化 | 後続 |
...
Observation Payloadの列定義は `docs/03_LOG_FORMAT.md` および `docs/06_GAS_API_SPEC.md` を正とする。現時点の確定フィールドとPROPOSEDフィールドの区別は `docs/03_LOG_FORMAT.md` を正とし、GAS本実装前に確定・除外判定を行う。未確定項目を「確定済み」と扱わない。
...
| docs/03_LOG_FORMAT.md | Spreadsheet / ログ仕様、確定Payloadフィールド、ERROR_LOG基準 |
| docs/04_STATE_MACHINE.md | 状態遷移 |
| docs/05_WIRING_DIAGRAM.md | 配線定義 |
| docs/06_GAS_API_SPEC.md | GAS API仕様、標準レスポンス、Calendar/Poem取得応答 |
...
| IP5306実モジュール仕様 | PROPOSED | 型番、出力容量、端子仕様、発熱、USB Presence検出方法は実機確認待ち |
| IP5306性能検証 | PROPOSED | 停電・復電切替、負荷応答、熱特性は実機評価で確認 |
...
| 2026-06-19 | Phase説明を簡略化し、詳細をCURRENT_STATUSへ委譲 | 最新査読でのREADME冗長性指摘への対応 |
| 2026-06-19 | リポジトリ構成を現在構成と将来構成へ分離 | 実構成と予定構成の混在防止 |
| 2026-06-19 | IP5306の確定度を採択・実モジュール・性能検証に分離 | PROPOSED/CONFIRMEDの意味明確化 |
